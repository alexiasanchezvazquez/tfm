...
use "$path/Data/panel_2011_2017.dta", clear
...
egen id=group(imputation h_2017)
...
bys imputation h_2017: egen mark_sum=sum(mark) 
drop if mark_sum!=0
...
replace mark=1 if pan_`j'==`i' & year==2017
} 
}
bys imputation h_2017: egen mark_sum=sum(mark) 
drop if mark_sum!=2
...
g couple=0
forvalues i=1/9{
		replace couple=`i' if p1_3_`i'==2 & year==2014
}

bys imputation h_2017: egen couple_sum=sum(couple)


g mark=0
forvalues j=1/9{
forvalues i=1/9{
	replace mark=1 if couple_sum==`i' & pan_`j'==`i' & year==2017
	replace mark=1 if couple_sum==`i' & pan_`i'!=. & pan_`i'!=0 & year==2014
	}
}
bys imputation h_2017: egen mark_sum=sum(mark)
...
bysort imputation h_2017: g pan_1_2014=pan_1[2]

g mark=0
forvalues i=2/9{
   replace mark=1 if pan_1_2014==`i' & p1_3_`i'!=2 & year==2011 
}
bys imputation h_2017: egen mark_sum=sum(mark)
drop if mark_sum!=0
drop pan_1_2014
...
forvalues j=1/9{
forvalues i=2011(3)2017{
g pan_`j'_`i'=pan_`j' if year==`i'
bys imputation h_2017: egen pan_`j'_`i'_prueba=sum(pan_`j'_`i')  
...
g `var'`i'_intermedio=`var'`i' if year==2014
}
}


* Linking 2011 and 2014
forvalues i=1/9{
forvalues j=1/9{
foreach var in `vlist'{
replace `var'`i'_intermedio=`var'`j' if pan_`i'_2014==`j' & year==2011
}
}
}

* Linking 2017 and 2014
forvalues i=1/9{
forvalues j=1/9{
foreach var in `vlist'{
replace `var'`i'_intermedio=`var'`j' if pan_`j'_2017==`i' & year==2017
}
}
}
...
forvalues i=1/9{
foreach var in `vlist'{
g `var'`i'_prueba=`var'`i'_intermedio if year==2011 | year==2014 | year==2017
}
}
...
g pareja=0
forvalues i=2/9{
replace pareja=`i' if p1_3_`i'==2 & year==2014
}

bys imputation h_2017: egen pareja_prueba=sum(pareja)
...
bys imputation h_2017: egen p1_prueba_sd=sd(p1_1_1_prueba)
br imputation h_2017 year p1_1_1_prueba if p1_prueba_sd!=0
drop p1_prueba_sd

bys imputation h_2017: egen p1_pareja_sd=sd(p1_1_pareja)
br imputation h_2017 year p1_1_pareja if p1_pareja_sd!=0 & p1_pareja_sd!=.
drop p1_pareja_sd
...
bys h_2017: egen marksum=sum(mark)
...
g exo_inter=0
replace exo_inter=1 if p2_3<=2011 & year==2017

bys h_2017: egen exo_sum=sum(exo_inter)
g exo=0
replace exo=1 if exo_sum!=0

g signal_inter=0
replace signal_inter=1 if p2_1!=2
bys h_2017: egen signal_sum=sum(signal_inter)
...
bys imputation h_2017: egen ddsum=sum(pareja)
drop pareja
g pareja=0
...
save "$path/Data/panel_2011_2017_constrained.dta", replace