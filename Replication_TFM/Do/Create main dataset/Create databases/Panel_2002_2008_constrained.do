...
use "$path/Data/panel_2002_2008.dta", clear
...
bys imputation h_2008: egen mark_sum=sum(mark) 
drop if mark_sum!=0
...
forvalues i=1/9{
    forvalues j=1/9{
replace mark=1 if pan_`i'==1 & year==2005
replace mark=1 if pan_`j'==`i' & year==2008
} 
}
bys imputation h_2008: egen mark_sum=sum(mark) 
drop if mark_sum!=2
...
g mark=0
forvalues i=1/9{
replace mark=1 if pan_`i'==1 & year==2008
} 
bys imputation h_2008: egen mark_sum=sum(mark) 
drop if mark_sum==0
...
g couple=0
forvalues i=1/9{
		replace couple=`i' if p1_3_`i'==2 & year==2005
}

bys imputation h_2008: egen couple_sum=sum(couple)


g mark=0
forvalues j=1/9{
forvalues i=1/9{
	replace mark=1 if couple_sum==`i' & pan_`j'==`i' & year==2008
	replace mark=1 if couple_sum==`i' & pan_`i'!=. & pan_`i'!=0 & year==2005
	}
}
bys imputation h_2008: egen mark_sum=sum(mark)
...
sort imputation h_2008 year
bysort imputation h_2008: g pan_1_2005=pan_1[2]

g mark=0
forvalues i=2/9{
   replace mark=1 if pan_1_2005==`i' & p1_3_`i'!=2 & year==2002 
}
bys imputation h_2008: egen mark_sum=sum(mark)
drop if mark_sum!=0
drop pan_1_2005
...
forvalues j=1/9{
forvalues i=2002(3)2008{
g pan_`j'_`i'=pan_`j' if year==`i'
bys imputation h_2008: egen pan_`j'_`i'_prueba=sum(pan_`j'_`i')  
...
g `var'`i'_intermedio=`var'`i' if year==2005
}
}


* Linking 2002 and 2005
forvalues i=1/9{
forvalues j=1/9{
foreach var in `vlist'{
replace `var'`i'_intermedio=`var'`j' if pan_`i'_2005==`j' & year==2002
}
}
}

* Linking 2008 and 2005
forvalues i=1/9{
forvalues j=1/9{
foreach var in `vlist'{
replace `var'`i'_intermedio=`var'`j' if pan_`j'_2008==`i' & year==2008
}
}
}
...
forvalues i=1/9{
foreach var in `vlist'{
g `var'`i'_prueba=`var'`i'_intermedio if year==2002 | year==2005 | year==2008
}
}
...
g pareja=0
forvalues i=2/9{
replace pareja=`i' if p1_3_`i'==2 & year==2005
}

bys imputation h_2008: egen pareja_prueba=sum(pareja)
...
bys imputation h_2008: egen p1_prueba_sd=sd(p1_1_1_prueba)
br imputation h_2008 year p1_1_1_prueba if p1_prueba_sd!=0  
drop p1_prueba_sd

bys imputation h_2008: egen p1_pareja_sd=sd(p1_1_pareja)
br imputation h_2008 year p1_1_pareja if p1_pareja_sd!=0 & p1_pareja_sd!=.
drop p1_pareja_sd




replace p1_1_1_prueba=1 if h_2008==723 & year==2005
replace p1_1_1_prueba=2 if h_2008==1912 & year==2002
replace p1_1_1_prueba=2 if h_2008==3019 & year==2005
replace p1_1_1_prueba=2 if h_2008==3175 & year==2002

replace p1_1_pareja=2 if h_2008==723 & year==2005
replace p1_1_pareja=1 if h_2008==1912 & year==2002
replace p1_1_pareja=1 if h_2008==3019 & year==2005
replace p1_1_pareja=1 if h_2008==3175 & year==2002
replace p1_1_pareja=1 if h_2008==4437 & year==2002
replace p1_1_pareja=2 if h_2008==5382 & year==2002
...
bys h_2008: egen marksum=sum(mark)
...
g exo_inter=0
replace exo_inter=1 if p2_3<=2002 & year==2008

bys h_2008: egen exo_sum=sum(exo_inter)
g exo=0
replace exo=1 if exo_sum!=0

g signal_inter=0
replace signal_inter=1 if p2_1!=2
bys h_2008: egen signal_sum=sum(signal_inter)
...
p6_63c4_9 p6_63c5_9 p6_63c6_9 p6_63c7_9 p6_63c8_9  expect_9 p6_13_1 p6_13_2 p6_13_3 p6_13_4 p6_13_5 p6_13_6 p6_13_7 p6_13_8 p6_13_9



bys imputation h_2008: egen ddsum=sum(pareja)
drop pareja
g pareja=0
...
save "$path/Data/panel_2002_2008_constrained.dta", replace