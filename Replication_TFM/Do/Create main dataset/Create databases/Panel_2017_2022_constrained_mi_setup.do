...
use "$path/Data/panel_2017_2022_constrained.dta", clear

**********************************************************************
**********************************************************************
** Variables and multiple imputation (2017-2022)
**********************************************************************
**********************************************************************
...
replace sector=1 if p6_4_1_prueba==1 & year==2022 & employment==1
replace sector=2 if p6_4_1_prueba==2 & year==2022 & employment==1 | p6_4_1_prueba==3 & year==2022 & employment==1 | p6_4_1_prueba==4 & year==2022 & employment==1 | p6_4_1_prueba==5 & year==2022 & employment==1
replace sector=3 if p6_4_1_prueba==6 & year==2022 & employment==1
replace sector=4 if p6_4_1_prueba>6 & year==2022 & p6_4_1_prueba!=. & employment==1
...
* -------------------------------------------------------------------------
* CREACIÓN DE VARIABLE SHOCK (Panel Irregular 2017-2020-2022)
* -------------------------------------------------------------------------
* Como el salto entre encuestas es irregular (2017 a 2020=3, 2020 a 2022=2), 
* creamos un indicador de ola temporal para no perder la imputación de 2022.
capture drop wave
gen wave = 1 if year == 2017
replace wave = 2 if year == 2020
replace wave = 3 if year == 2022

capture drop id
egen id=group(imputation h_2022)
...
drop wave employed_bin unemployed_bin shock_main shock_workers id
* -------------------------------------------------------------------------
...
replace constraints=1 if p3_12a==1 & year==2022 | p3_12b==1 & year==2022 | p3_13s1==4 & year==2022 | p3_13s2==4 & year==2022 | p3_13s3==4 & year==2022 | p3_13s4==4 & year==2022 | p3_13s5==4 & year==2022
...
* Precaution
g preca_1=1 if expect_1_prueba>50 & expect_1_prueba!=. & year==2017 | expect_1_prueba>50 & expect_1_prueba!=. & year==2020 | expect_1_prueba>50 & expect_1_prueba!=. & year==2022 
...
* Speculation
g spec_1=0
replace spec_1=1 if p2_36_1>=2017 & p2_36_1<=2020 | p2_36_2>=2017 & p2_36_2<=2020 | p2_36_3>=2017 & p2_36_3<=2020 | p2_33>3 & p2_33!=. 
...
* Chain: 2022 -> 2020 -> 2017 -> 2014 -> 2011
* Factor 2020-2022: 1.1263 (BdE)
...
foreach var of varlist riquezanet riquezabr actreales actfinanc vdeuda real_debt fin_debt p2_5 {
    ...
    g `var'_2020=`var' if year==2020
    
    * Deflactor 2020-2022 (Bank of Spain)
    replace `var'_2020=`var'/1.1263 if year==2022

    * Deflactor 2017-2020 (Bank of Spain)
    replace `var'_2017=`var'_2020/1.0144 if year==2020 | year==2022

    * Deflactor 2011-2017 (Bank of Spain) - chaining through 2014 to keep 2011 base
    g `var'_2011=`var'_2017/(1.0272 * 1.0205) if year==2017 | year==2020 | year==2022

    drop `var' `var'_2017 `var'_2020
    rename `var'_2011 `var'_real
}

* Outliers
g dd=1 if nonfin_inc_real<=0 
bys h_2022: egen ddsum=sum(dd)
drop if ddsum!=0
drop dd ddsum
...
foreach var of varlist  gage gender ownership size liqcons risk  members_working kids health_1  edu_h_1 edu_m_1   job_h_1  income_dev income_expect cpi* mrenthog_real renthog_real  consumption_real actfinanc_real actreales_real  vdeuda_real real_debt_real fin_debt_real riquezabr_real riquezanet_real durable nonfin_inc constraints precautionary contract sector employment years_house gen_constraints pending_loans number_pending_loans years_pending interest_rate interest_rate_type debt_service_real  debt_service_real_assets  debt_service_fin_assets years_pending_weight share_remaining_mortgage share_fixed_rate p2_5_real refin spec_* collateral shock {
				...
				confirm numeric variable `var'
					{
						tempvar sd
						quietly bysort h_2022 year : egen `sd'=sd(`var') 
						quietly replace `var'=. if (`sd'>0 & `sd' <.) & imputation==0 
						drop `sd'
				  } 
				}

save "$path/Data/panel_2017_2022_constrained_mi", replace
