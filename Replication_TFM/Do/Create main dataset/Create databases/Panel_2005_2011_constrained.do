...
use "$path/Data/panel_2005_2011.dta", clear
...
bys imputation h_2011: egen mark_sum=sum(mark) 
drop if mark_sum!=0
...
replace mark=1 if pan_`j'==`i' & year==2011
} 
}
bys imputation h_2011: egen mark_sum=sum(mark) 
drop if mark_sum!=2
...
g couple=0
forvalues i=1/9{
		replace couple=`i' if p1_3_`i'==2 & year==2008
}

bys imputation h_2011: egen couple_sum=sum(couple)


g mark=0
forvalues j=1/9{
forvalues i=1/9{
	replace mark=1 if couple_sum==`i' & pan_`j'==`i' & year==2011
	replace mark=1 if couple_sum==`i' & pan_`i'!=. & pan_`i'!=0 & year==2008
	}
}
bys imputation h_2011: egen mark_sum=sum(mark)
...
bysort imputation h_2011: g pan_1_2008=pan_1[2]

g mark=0
forvalues i=2/9{
   replace mark=1 if pan_1_2008==`i' & p1_3_`i'!=2 & year==2005 
}
bys imputation h_2011: egen mark_sum=sum(mark)
drop if mark_sum!=0
drop pan_1_2008
...
forvalues j=1/9{
forvalues i=2005(3)2011{
g pan_`j'_`i'=pan_`j' if year==`i'
bys imputation h_2011: egen pan_`j'_`i'_prueba=sum(pan_`j'_`i')  
...
p1_7_ p1_5_ p6_3_ p6_4_ p6_13_ p6_63c1_ p6_63c2_ p6_63c3_ p6_63c4_ p6_63c5_ p6_63c6_ p6_63c7_ p6_63c8_  expect_ 


forvalues i=1/9{
foreach var in `vlist'{
g `var'`i'_intermedio=`var'`i' if year==2008
}
}


* Linking 2005 and 2008
forvalues i=1/9{
forvalues j=1/9{
foreach var in `vlist'{
replace `var'`i'_intermedio=`var'`j' if pan_`i'_2008==`j' & year==2005
}
}
}

* Linking 2011 and 2008
forvalues i=1/9{
forvalues j=1/9{
foreach var in `vlist'{
replace `var'`i'_intermedio=`var'`j' if pan_`j'_2011==`i' & year==2011
}
}
}
...
forvalues i=1/9{
foreach var in `vlist'{
g `var'`i'_prueba=`var'`i'_intermedio if year==2005 | year==2008 | year==2011
}
}
...
g pareja=0
forvalues i=2/9{
replace pareja=`i' if p1_3_`i'==2 & year==2008
}

bys imputation h_2011: egen pareja_prueba=sum(pareja)
...
bys imputation h_2011: egen p1_prueba_sd=sd(p1_1_1_prueba)
br imputation h_2011 year p1_1_1_prueba if p1_prueba_sd!=0
drop p1_prueba_sd

bys imputation h_2011: egen p1_pareja_sd=sd(p1_1_pareja)
br imputation h_2011 year p1_1_pareja if p1_pareja_sd!=0 & p1_pareja_sd!=.
drop p1_pareja_sd




replace p1_1_1_prueba=1 if h_2011==4127 & year==2005
replace p1_1_pareja=2  if h_2011==4127 & year==2005
replace p1_1_1_prueba=2 if h_2011==4501 & year==2005
replace p1_1_pareja=1 if h_2011==4501 & year==2005
replace p1_1_1_prueba=2 if h_2011==2430 & year==2005
...
bys h_2011: egen marksum=sum(mark)
...
g exo_inter=0
replace exo_inter=1 if p2_3<=2005 & year==2011

bys h_2011: egen exo_sum=sum(exo_inter)
g exo=0
replace exo=1 if exo_sum!=0

g signal_inter=0
replace signal_inter=1 if p2_1!=2
bys h_2011: egen signal_sum=sum(signal_inter)
...
bys imputation h_2011: egen ddsum=sum(pareja)
drop pareja
g pareja=0
...
save "$path/Data/panel_2005_2011_constrained.dta", replace