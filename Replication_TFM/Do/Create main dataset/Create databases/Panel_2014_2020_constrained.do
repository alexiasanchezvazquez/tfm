...
use "$path/Data/panel_2014_2020.dta", clear

****************************************************************************************************************************************
****************************************************************************************************************************************
****** Sample and variables selection (2014-2020)
****************************************************************************************************************************************
****************************************************************************************************************************************
...
bys imputation h_2020: egen mark_sum=sum(mark) 
drop if mark_sum!=0
...
forvalues i=1/9{
	forvalues j=1/9{
		replace mark=1 if pan_`i'==1 & year==2017
		replace mark=1 if pan_`j'==`i' & year==2020
	} 
}
bys imputation h_2020: egen mark_sum=sum(mark) 
drop if mark_sum!=2
...
sort imputation h_2020
by imputation h_2020: egen mark_sum=sum(mark) 
drop if mark_sum==0

drop mark mark_sum
g couple=0
forvalues i=1/9{
	replace couple=`i' if p1_3_`i'==2 & year==2017
}
bys imputation h_2020: egen couple_sum=sum(couple)

g mark=0
forvalues j=1/9{
	forvalues i=1/9{
		replace mark=1 if couple_sum==`i' & pan_`j'==`i' & year==2020
		replace mark=1 if couple_sum==`i' & pan_`i'!=. & pan_`i'!=0 & year==2017
	}
}
bys imputation h_2020: egen mark_sum=sum(mark)
...
bysort imputation h_2020: g pan_1_2017=pan_1[2]

g mark=0
forvalues i=2/9{
   replace mark=1 if pan_1_2017==`i' & p1_3_`i'!=2 & year==2014 
}
bys imputation h_2020: egen mark_sum=sum(mark)
drop if mark_sum!=0
drop pan_1_2017
...
forvalues j=1/9{
	forvalues i=2014(3)2020{
		g pan_`j'_`i'=pan_`j' if year==`i'
		bys imputation h_2020: egen pan_`j'_`i'_prueba=sum(pan_`j'_`i')  
...
forvalues i=1/9{
	foreach var in `vlist'{
		g `var'`i'_intermedio=`var'`i' if year==2017
	}
}

* Linking 2014 and 2017
forvalues i=1/9{
	forvalues j=1/9{
		foreach var in `vlist'{
			replace `var'`i'_intermedio=`var'`j' if pan_`i'_2017==`j' & year==2014
		}
	}
}

* Linking 2020 and 2017
forvalues i=1/9{
	forvalues j=1/9{
		foreach var in `vlist'{
			replace `var'`i'_intermedio=`var'`j' if pan_`j'_2020==`i' & year==2020
		}
	}
}
...
forvalues i=1/9{
	foreach var in `vlist'{
		g `var'`i'_prueba=`var'`i'_intermedio if year==2014 | year==2017 | year==2020
	}
}
...
g pareja=0
forvalues i=2/9{
	replace pareja=`i' if p1_3_`i'==2 & year==2017
}

bys imputation h_2020: egen pareja_prueba=sum(pareja)
...
bys imputation h_2020: egen p1_prueba_sd=sd(p1_1_1_prueba)
br imputation h_2020 year p1_1_1_prueba if p1_prueba_sd!=0
drop p1_prueba_sd

bys imputation h_2020: egen p1_pareja_sd=sd(p1_1_pareja)
br imputation h_2020 year p1_1_pareja if p1_pareja_sd!=0 & p1_pareja_sd!=.
drop p1_pareja_sd
...
bys h_2020: egen marksum=sum(mark)
drop if marksum!=0

* RESTRICCIÓN CLAVE: Compra de vivienda antes del periodo base (<=2014 para el panel 2014-2020)
g exo_inter=0
replace exo_inter=1 if p2_3<=2014 & year==2020

bys h_2020: egen exo_sum=sum(exo_inter)
g exo=0
replace exo=1 if exo_sum!=0

g signal_inter=0
replace signal_inter=1 if p2_1!=2
bys h_2020: egen signal_sum=sum(signal_inter)
...
bys imputation h_2020: egen ddsum=sum(pareja)
drop pareja
g pareja=0
replace pareja=1 if ddsum!=0 & ddsum!=.
drop ddsum

save "$path/Data/panel_2014_2020_constrained.dta", replace
